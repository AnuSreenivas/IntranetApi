...
{
    public class canteenHeadCountController : ApiController
    {
        [HttpGet]
        public canteenHeadCountClass[] canteenHeadCountClass1()
        {
            try
            {
                ...
                using (con)
                {
                    SqlCommand cmd = new SqlCommand("HCMDB..avt_sp_headcount_ion_list_total", con);
                    ...
                    List<canteenHeadCountClass> arrayofArray = new List<canteenHeadCountClass>();
                    canteenHeadCountClass objArray;
                    //int i = 0;
                    while (reader.Read())
                    {
                        objArray = new canteenHeadCountClass(Convert.ToString(reader[0]), Convert.ToInt16(reader[1]));
