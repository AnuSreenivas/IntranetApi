...
{
    public class biopsuserController : ApiController
    {
        [HttpPost]
        public csinsertreturn InsertUser(csuserClass usr)
        {
            try
            {
                ...
                using (con)
                {
                    SqlCommand cmd = new SqlCommand("vms..avt_ion_insert_usersrights", con);
        ...
        public cspagesclass[] GetAllPages(int val)
        {
            try
            {
                ...
                using (con)
                {
                    SqlCommand cmd = new SqlCommand("vms..avt_ion_pagelst", con);
                    cmd.CommandType = CommandType.StoredProcedure;
        ...
        public csuserClass[] GetAllUsers()
        {
            try
            {
                ...
                using (con)
                {
                    SqlCommand cmd = new SqlCommand("vms..avt_ion_userlst", con);
                    cmd.CommandType = CommandType.StoredProcedure;
        ...
        public int[] getuserrights(int userid)
        {
            try
            {
                ...
                using (con)
                {
                    SqlCommand cmd = new SqlCommand("vms..avt_ion_userrightslst", con);
                    cmd.CommandType = CommandType.StoredProcedure;
        ...
        [HttpGet]
        public csuserClass userlogin(string username, string password, string uuid, string manuf)//, string siminfo)
        {
            try
            {
                string cs = ConfigurationManager.ConnectionStrings["avt_data1"].ConnectionString;
